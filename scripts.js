...
// Please refresh the page before any update - this will update correctly
// otherwise will always udpdate the first Instance updated..
...
function updateUser(userId) {
  console.log(userId +" ID - at the begin - in updateUser");
    if(updated===1){
    // before we can add a user we need to form the data that we need to store
    // we use the values in the inputs to generate the final object that will be the payload for the server call
    const userToUpdate = {
    ...
    fetch("https://contact-agenda-rest-api.herokuapp.com/users/" + userId, {
      ...
      console.log('updated after made 0');
      console.log(userId +" ID - at the end - in updateUser");
      //-----------
  ...
  else location.reload(); // it is useless 
